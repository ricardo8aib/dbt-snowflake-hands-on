...
dim_date as (
	...
	order by date asc),
payments as (select 
	ROW_NUMBER() OVER(ORDER BY o_payments.order_id, o_payments.payment_sequential ASC) AS payment_id,
	o_payments.order_id,
	o_payments.payment_sequential,
	o_payments.payment_type,
	o_payments.payment_installments,
	o_payments.payment_value
...
select 
	...
	(select sum(p.payment_value) from payments as p where orders.order_id = p.order_id) as amount_paid
from {{ ref('stg_orders') }} as orders
left join {{ ref('stg_customers') }} as customers on orders.customer_id = customers.customer_id
left join (select date_id, date from dim_date) as p_timestamp on orders.order_purchase_timestamp = p_timestamp.date
left join (select date_id, date from dim_date) as approved on orders.order_approved_at = approved.date
left join (select date_id, date from dim_date) as delivered_ca on orders.order_delivered_carrier_date = delivered_ca.date
left join (select date_id, date from dim_date) as delivered_cu on orders.order_delivered_customer_date = delivered_cu.date
left join (select date_id, date from dim_date) as estimated on orders.order_estimated_delivery_date = estimated.date